...
When the prefetch buffer is empty, and a brnahc is done tyo the next instruction that would be read by the prefetcher, the instruction fetch is done with non-sequential timing (even though the prefetcher starts at the same time with sequential timing.)
